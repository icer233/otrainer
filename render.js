    ...
    out += `<div class="student-box">
      ...
      <div class="student-header">
        <div class="student-name" style="display:flex;align-items:center;gap:6px;">
          ${s.name}
          <button class="edit-name-btn" data-idx="${game.students.indexOf(s)}" title="修改姓名" style="margin-left:6px;padding:2px 8px;font-size:12px;line-height:1.2;border-radius:4px;border:1px solid #ddd;background:#f7fafc;cursor:pointer;">修改</button>
  ...
  if (studentListEl && !studentListEl._evictDelegated) {
    ...
    studentListEl.addEventListener('click', function (e) {
      ...
      if (editBtn) {
        ...
        if (isNaN(idx)) return;
        const oldName = game.students[idx].name;
        const newName = prompt('请输入新的学生姓名：', oldName);
        if (newName && newName.trim() && newName !== oldName) {
          game.students[idx].name = newName.trim();
      ...
      try {
        if (typeof s.triggerTalents === 'function') {
          ...
          for (const r of results) {
            if (!r || !r.result) continue; const out = r.result; if (typeof out === 'object') {
